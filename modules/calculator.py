def calculate_job_quote(
    ...
    gph = tractor_data.get("fuel_burn_hour", 3.0)
    tractor_wear = tractor_data.get("wear_cost", 5.0)
    tractor_hp = tractor_data.get("hp", tractor_data.get("HP", 0))

    # Math Engine #


    effective_width = max(working_width - float(overlap_ft), 0.5)
    field_capacity = (effective_width * (working_speed) * float(field_efficiency)) / 8.25
    job_hours = acres / field_capacity if field_efficiency > 0 else 0.0

    fuel_used = job_hours * gph
    fuel_cost = fuel_used * float(fuel_price)

    total_wear_rate = implement_wear + tractor_wear
    wear_cost = job_hours * total_wear_rate

    labor_cost = float(hourly_rate) * job_hours
    hauling_cost = float(loading_fee) + (float(round_trip_miles) * float(mileage_rate))
    ...
    hourly_billing_quote = (machine_rate_hr + float(hourly_rate)) * job_hours
    ...
    total_quote = max(raw_quote, margin_based_quote, float(min_job_charge))
